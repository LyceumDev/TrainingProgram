...
you_need: the traced candidate list with its dispositions and decision owners, the acceptance decisions made so far, and the house template
environment: browser
...
1. **Gather what the draft rests on**
   The traced requirements list from the [requirements guide](~/guides/02-extract-requirements-in-the-authors-own-words.html) with its dispositions and decision owners, the acceptance decisions made so far, the house template if there is one, and an example the organization considers good.
...
4. **Ask for the traceability table and the consistency checks**
   Identifier, statement, source, the decision that accepted it, its label. Then: statements that contradict each other, terms used two ways, requirements with no acceptance criterion, and everything still {{label kind=open}}, grouped by who can close it. Do not supply a missing acceptance criterion as established fact: if the sources do not provide one, mark it {{label kind=open}} or propose one as {{label kind=inferred}} for approval by its named owner.
5. **Review every requirement's wording and follow the traces**
   Trace every high-impact or newly changed requirement, plus five selected at random. If any sampled trace fails, check the complete table. Hunt the words that hide decisions. Part 2 below has the checks.
...
4. **Acceptance criteria** How each requirement will be checked, traced to a source or labeled Inferred or Open, with the person authorized to approve it.
...
- You review every requirement's wording and approve it where you hold that authority; otherwise you route it to the named requirement owner. The assistant may draft or propose changes; you remain accountable for the text's use and preserve contribution provenance where material.
...
- Every high-impact or newly changed requirement, plus five at random, each followed back through the table to its source sentence. If any sampled trace fails, check the complete table.
...
You provide
:   "The traced candidate list with its dispositions and owners, the scope decision from the operations lead, and the house template. Readers: the build team and the QA lead."
...
You correct
:   R-12 appeared during drafting with no source and no decision behind it. You remove it and note why in the change history.
