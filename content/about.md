...
Designed toward WCAG 2.2 AA and the Revised Section 508 Standards; validation is in progress. The build checks the theme colors for contrast in both themes, page structure, and link integrity, and the pages are built for keyboard operation, visible focus, reduced motion, and descriptive links. A documented keyboard and screen-reader pass of the whole route is required before the first participants use the portal, and this page will say so when it is complete. If something does not work for you, tell the [program guide](~/support/index.html); we treat accessibility defects as priority fixes.
